...
public class Bot extends TelegramLongPollingBot {
  ...
  final List<String> sequenceOfTaskQuestions = Arrays
      .asList("Квартира", "Имя", "Телефон", "Описание проблемы");
  ...
  private void showMainMenu(String chatId, String message) {
    ...
    switch (message) {
      case "Контакты ТС":
        sendSpecifiedText(chatId, contactInfo);
        break;
      case "Новая заяка":
        showNewTaskMenu(chatId);
        break;
      default:
        sendSpecifiedText(chatId, greetingText);
    }
  }
  ...
  private void showNewTaskMenu(String chatId) {
    ...
    try {
      ...
      logger.info(new ParameterizedMessage("Text: \"{}\" was sent to chatId: {}",
          taskProgress.get(CURRENT_QUESTION), chatId));
    } catch (TelegramApiException e) {
      logger
          .error(new ParameterizedMessage("Error during sending of a message to chatId {}", chatId),
              e);
    }
  }
  ...
  public void sendNotificationToAdmins(TaskNotification notification) {
    sendSpecifiedText(adminChatId, notification.toString(), false);
  }
